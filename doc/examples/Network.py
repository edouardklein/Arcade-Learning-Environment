...
class Network:
    ...
    def getAbstractType_list(self):
        ...
        if numDoubles > 0:
            ...
            returnStruct.doubleArray = list(
                struct.unpack("!%dd" % (numDoubles), s))
        if numChars > 0:
            s = self.recvBuffer.read(numChars * kCharSize)
            returnStruct.charArray = list(
                struct.unpack("!%dc" % (numChars), s))
        return returnStruct
    ...
    def putString(self, value):
        ...
        self.sendBuffer.write(value.encode('utf-8'))
    ...
    def putAbstractType(self, theItem):
        ...
        if len(theItem.doubleArray) > 0:
            self.sendBuffer.write(
                struct.pack("!%dd" % (len(theItem.doubleArray)), *(theItem.doubleArray)))
        if len(theItem.charArray) > 0:
            self.sendBuffer.write(
                struct.pack("!%dc" % (len(theItem.charArray)), *(theItem.charArray)))
